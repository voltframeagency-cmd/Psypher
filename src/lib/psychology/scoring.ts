...
export class PsychologyEngine {
  ...
  static generateHybridReport(
    ...
    // Vector B: Linguistic (if text provided)
    const hasTextSample = !!(textSample && textSample.trim().split(/\s+/).length >= 100);
    const linguistic = hasTextSample ? this.analyzeLinguistic(textSample!) : null;

    // Vector C: Congruency (only if both vectors exist)
    const congruency = (hasTextSample && linguistic)
      ? this.calculateCongruency(bfi, darkTriad, linguistic, textSample!)
      : null;
